/*
    * VIEW (뷰)
    ...
    임시 테이블과 같은 존재(실제 데이터가 저장되는 게 아니라, 논리적으로 저장되어 있음!)
*/

-- * 사원 (EMPLOYEE), 부서 정보(DEPARTMENT) , 지역(LOCATION), 국가 (NATIONAL) *
...
-- 한국에서 근무하는 사원 정보 조회
SELECT * FROM VW_EMPLOYEE
WHERE VATIONAL_NAME = '한국';

-- 러시아에서 근무하는 사원 정보 조회
SELECT * FROM VW_EMPLOYEE
WHERE VATIONAL_NAME = '러시아';
...
AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME, BONUS
 ...
 SELECT EMP_ID, JOB_NAME,DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남',2, '여'),
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남',2, '여') "성별",
 EXTRACT(YEAR FROM SYSDATE)- EXTRACT(YEAR FROM HIRE_DATE)근무년수 FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE);
...
--------------------------------------------------------------------------------
 /*
    생성된 뷰를 통해서 DML(INSERT/UPDATE/DELETE)를 사용해보기
 ...
 */
 ...
 /*
    * DML 명령어로 조작이 불가능한 경우가 많음!
    ...
    1) VIEW에 정의되지 않은 컬럼을 조작하려고 하는 경우
    2) VIEW에 정의되어 있지 않고 베이스테이블에서 NOT NULL 제약조건이 지정되어 있는 경우
    3) 산술연산식 또는 함수식으로 정의가 되어 있는 경우
    4) DISTINCT 중복 구문이 포함되어 있는 경우
    5) JOIN 을 이용하여 여러 테이블을 연결시킨 경우
    
    => 뷰는 대부분 조회의 용도로 사용한다. 그렇기 때문에 되도록이면 DML을 사용하지 말자...!
 */
...
/*
    * VIEW 옵션 *
    ...
    - OR REPLACE : 기존에 동일한 이름의 뷰가 있을 경우 갱신하고, 존재하지 않을 경우 생성
    - FORCE | NOFORCE
...
SELECT * FROM VW_TEMP; --> 뷰에 오류가 있다고 함!
...
AS SELECT *
 ...
 -- 204번 사원의 급여를 200만원으로 변경 (뷰 생성 조건에 맞지 않는 경우)
 UPDATE VW_EMP
    SET SALARY = 2000000 
    WHERE EMP_ID = 204;
