...
-- 모든 직원의 평균 급여를 계산 -> 평균값 계산 : AVG
-- * 반올림 : ROUND, 올림 : CEIL, 내림(버림) : FLOOR
...
-- , CONSTRAINT PRJ_PK PRIMARY KEY(PROJECT_ID)
);
...
ROLLBACK;       --변경사항 취소(DB에 적용하지 않음)

-- EMPLOYEES 테이블에서 급여가 5000 이하인 직원들을 삭제
DELETE FROM EMPOLYEES WHERE SALARY <= 5000;
