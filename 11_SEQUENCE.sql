...
/*
    * 시퀀스 생성하기 *
    ...
    [MAXVALUE 숫자]   --> 최댓값 [기본값,,, 엄청 큼]
    [MINVALUE 숫자]   --> 최솟값 [기본값: 1]
    [CYCLE | NOCYCLE] --> 값의 순환여부 [기본값: NOCYCLE]
                     --> CYCLE : 시퀀스 값이 최댓값에 도달하면 최소값으로 다시 순환하도록 설정
    ...
    [NOCACHE | CACHE 숫자] --> 캐시메모리 할당 여부 [기본값 : CACHE 20]
                          --> 캐시메모리 : 미리 발생될 값들을 생성해서 저장해두는 공간
                                            ...
                                            캐시메모리라는 공간에 미리 생성된 값들을 가져다가 사용
                                            (미리 만들어놨기 때문에 속도가 빠름)
     [참고]
     - 이름 지어주기 -
     * 테이블 : TB_
...
-- 시작번호를 300, 증가값은 5, 최댓값은 310, 순환 X, 캐시메모리 X
...
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;     --오류 발생!! NEXTVAL을 한번도 수행하지 않은 이상 CURRVAL 는 사용할 수 없음!
...
SELECT * FROM USER_SEQUENCES;
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL;     -- 315 ==> 오류 발생! 최댓값이 310으로 설정되어 있어 그 이상의 값이 조회되므로 오류 발생
...
INSERT INTO EMPLOYEE (EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
    VALUES (SEQ_EID.NEXTVAL, '조건순', '990928-1111111', 'J4' ,SYSDATE);
