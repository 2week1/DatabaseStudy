...
-- 부서 내 직급별 사원수, 급여 총 합 조회
SELECT DEPT_CODE, JOB_CODE, COUNT(*), SUM(SALARY) "급여 총 합"
FROM EMPLOYEE
...
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
WHERE AVG(SALARY) >= 3000000 -- 사원의 급여만 조건으로 제시함!
GROUP BY DEPT_CODE;
...
-- 직급 별 직급 코드, 총 급여 합 조회(단, 직급별 급여 합이 1000만원 이상인 직급만 조회)
...
/*
    ...
    3) 정렬하고자 한다면 ORDER BY절은 마지막에 작성해야 함
*/
