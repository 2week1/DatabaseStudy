/*
    * DCL (Data Control Langage) : 데이터 제어 언어
...
/*
    ...
    - RESOURCE : 자원 관리(특정 객체 생성 권한) (CREATE TEABLE, CREATE VIEW, ...)
*/
...
/*
    ...
    * 트랜잭션 종류
        ...
        - ROLLBACK : 트랜잭션에 담겨 있는 변경사항들을 삭제(취소) 하고,
...
-- EMP01 테이블 삭제 
DROP TABLE EMP01;       --> DDL(데이터 정의) 트랜잭션 처리 X.
...
-- 사번이 217, 214인 사원 삭제 -> DML(데이터 조작) 트랜잭션 처리 O
DELETE FROM EMP01
...
ROLLBACK;           -- 마지막 커밋 위치로 돌아감 (217, 214 사원이 삭제된 시점)
--------------------------------------------------------------------------------
...
COMMIT;     -- SP 시점으로 돌아간 뒤 COMMIT 을 했기 때문에 DELETE 208, 209, 210만 DB에 적용됨
--------------------------------------------------------------------------------
...
-- DDL 문 ( CREATE, ALTER, DROP )을 사용하게 되면 기존 트랜잭션에 저장된 변경사항들이 무조건 반영됨(COMMIT)
-- DDL 문 사용 전 DML로 사용된 쿼리들이 있다면, 확실하게 처리(COMMIT/ROLLBACK) 한 후에 DDL을 사용해야 함!
