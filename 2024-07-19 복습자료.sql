...
-- 나이가 30대인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE BIRTHDATE BETWEEN '85/01/01' AND '94/12/31' ;
        --EXTRACT (YEAR FROM SYSDATE) - EXTRACT (YEAR FROM BIRTHDATE) + 1 "나이" FROM CUSTOMER;

-- 광역시에 거주 중인 사람들의 정보 조회
SELECT ADDRESS FROM CUSTOMER WHERE ADDRESS LIKE '%광역시%';

-- 이름이 2자인 사람들의 정보 조회
SELECT * FROM CUSTOMER WHERE NAME LIKE '__';

-- 이름, 생년월일, 나이 정보 조회
SELECT NAME 이름, BIRTHDATE 생년월일, FLOOR((SYSDATE - BIRTHDATE) / 365) + 1 나이 FROM CUSTOMER;
