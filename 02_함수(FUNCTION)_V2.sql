...
-- 남자 사원들의 총 급여
SELECT SUM(SALARY) "남자사원들의 총 급여" FROM EMPLOYEE 
WHERE SUBSTR(EMP_NO,8,1) IN ('1','3');

-- 여자 사원들의 총 급여
SELECT SUM(SALARY) "여자사원들의 총 급여" FROM EMPLOYEE 
WHERE SUBSTR(EMP_NO,8,1) IN ('2','4');

--부서코드가 'D5'인 사원들의 총 연봉

SELECT SUM(SALARY * 12) "부서코드 D5인 사원들의 총 연봉" FROM EMPLOYEE
WHERE DEPT_CODE ='D5';
...
/*
    ...
    [표현법]
            ...
            COUNT(컬럼) : 해당 컬럼값이 NULL이 아닌 것만 행의 개수로 세어 반환
            COUNT(DISTINCT 컬럼) : 해당 컬럼값의 중복을 제거한 후의 행의 개수를 세어 반환
                => 중복 제거 시, NULL은 포함하지 않고 개수가 세어짐!
*/  
...
-- 보너스 받는 사원 수 조회 1)
SELECT COUNT (BONUS) "보너스 받는 사원 수"FROM EMPLOYEE;
-- 보너스 받는 사원 수 조회 2)
