...
/*
        * 서브쿼리의 종류 *
        ...
        - 다중행 서브쿼리 : 서브쿼리의 수행 결과가 한 행이고 여러개의 컬럼일 때 (1행 N열)
        - 다중행 다중열 서브쿼리 : 서브쿼리의 수행 결과가 여러행 여러 컬럼일 때 (N행 N열)
...
/*
    ...
    > ANY (서브쿼리) : 여러 개의 결과값 중에서 하나라도 큰 경우가 있으면 조회
...
-- 하이유 사원과 같은 부서 코드, 같은 직급 코드에 해당하는 사원 정보 조회
...
FROM EMPLOYEE WHERE DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE
                                WHERE EMP_NAME ='하이유')
...
SELECT JOB_CODE, MANAGER_ID FROM EMPLOYEE
WHERE EMP_NAME = '박나라';

-- 2) 같은 직급코드, 같은 사수를 가지고 있는 사원 정보를 조회
SELECT EMP_NAME, JOB_CODE, MANAGER_ID
FROM EMPLOYEE WHERE(JOB_CODE, MANAGER_ID) = (SELECT JOB_CODE, MANAGER_ID FROM EMPLOYEE
                                            WHERE EMP_NAME = '박나라')
...
/*
    인라인 뷰 : 서브쿼리를 FROM 절에 사용하는 것
                => 서브쿼리의 수행 결과는 마치 테이블처럼 사용하는 것
*/

-- 사원들의 사번, 이름, 보너스 포함 연봉, 부서코드 조회
-- * 보너스 포함 연봉이 NULL이 아니어야 하고, 보너스 포함 연봉이 3000만원 이상인 사원들만 조회
--          => NVL : NULL 인 경우 다른 값으로 변경
SELECT EMP_ID 사번, EMP_NAME 이름, ( SALARY + (SALARY * NVL(BONUS, 0)))"보너스 포함 연봉", DEPT_CODE 부서코드 FROM EMPLOYEE
WHERE (SALARY + (SALARY * NVL(BONUS, 0))) * 12 >= 30000000
ORDER BY 3 DESC;

SELECT ROWNUM,사번,이름,"보너스 포함 연봉",부서코드 
FROM (SELECT EMP_ID 사번, EMP_NAME 이름, ( SALARY + (SALARY * NVL(BONUS, 0)))"보너스 포함 연봉", DEPT_CODE 부서코드 FROM EMPLOYEE
WHERE (SALARY + (SALARY * NVL(BONUS, 0))) * 12 >= 30000000
ORDER BY 3 DESC)
...
/*
    ...
    - DANSE() OVER(정렬기준)    :동일한 순위가 있더라도 그 다음 등수를 +1해서 순위 계산
...
FROM EMPLOYEE;
--> 공동 19인 2명 이후에 그 위의 순위가 20위로 표시된 (+1)

--> 상위 5명만 조회
SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE
WHERE ROWNUM <= 5;
-- WHERE 순위 <= 5; --> 오류 발생! WHERE 절에서는 별칭 사용 불가!
SELECT * FROM(SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE)
WHERE 순위 <= 5;
...
-- 1) ROWNUM 을 활용하여 급여가 가장 높은 5명을 조회하려 했으나, 제대로 조회가 되지 않았다.
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;

-- 문제점 (원인) : 정렬 전 5명이 추려졌음
-- 해결방안      :
SELECT ROWNUM, 이름, 급여
...
-- 2) 부서별 평균급여가 270만원을 초과하는 부서에 해당하는 부서코드, 부서별 총 급여합, 부서별 평균급여, 부서별 사원 수 조회
...
-- 문제점 (원인) : 부서별 평균급여를 확인해야 하는데, 사원 개개인의 급여를 확인했다.
-- 해결방안      :
SELECT DEPT_CODE, SUM(SALARY) "총합", FLOOR(AVG(SALARY)) AS "평균", COUNT(*) "인원수"
FROM EMPLOYEE
-- WHERE SALARY > 2700000
